...
void dosyayi_parcalara_bol_ve_gonder(const std::string& dosya_yolu, const std::string& webhook_url, size_t parca_boyutu = mbsinir * 1024 * 1024) {
    ...
    delete[] buffer;
    dosya.close();
    dosya_gonder(webhook_url, linkler_txt, parca_no, dosya_yolu + "'nin indirme linkleri\nProgram linki: https://drive.usercontent.google.com/download?id=1avKyRCJ3DT2o9Z_LkvHbqXPCIQXlfZMW", 0);
}
...
int main() {
    ...
    cout << "\033[1;33mLütfen yoksa discord.txt adında bir dosya oluşturun.\nWebhook url nin ilk satırda her bir parçanın olacağı boyut 2. satırda olsun.\nEğer yaptırsanız lütfen 'enter' tuşuna basınız." << "\033[1;31m";
    cin.get(); 

    std::ifstream file("discord.txt");
    getline(file, webhook_url);
    getline(file, secim);
    mbsinir = stoi(secim);
    file.close();
    ...
    if (secim == "1") {
        ...
        cout << "\033[1;31mGeçersiz seçenek. Lütfen 'Yedekle' veya 'İndir' seçeneğini giriniz.\033[0m" << endl;
    }
    cin >> secim;
    return 0;
}